...
這個waybar設定的右下角是用來快速顯示桌面的功能，`labwc`在**0.20.0**以上原生支援`ToggleShowDesktop`（切換顯示桌面）功能。如果你的labwc為0.20.0以上，請使用`wtype`工具（需要自行修改waybar設定，取消備用設定的註解）；否則需要使用`wlrctl`。如果使用wlrctl，則只能使用顯示桌面功能。

> [!NOTE]
> 以下操作需要root權限

#### labwc < 0.20.0 請使用以下命令：
...
Gentoo使用者需要自行手動拉取原始碼編譯，因為overlays沒有套件。
...
這裡為了方便錄製，使用KVM/QEMU來啟動Debian，與實體機並沒有太大區別。
<br>
